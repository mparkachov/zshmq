Describe 'topic send'
  ...
  ensure_dispatcher_running() {
    ...
    printf '%s\n' "$TOPIC_SEND_SPEC_DISPATCHER_PID" > "${ZSHMQ_CTX_ROOT}/bus.pid"
  }

  It 'publishes to the requested topic'
    TOPIC_SEND_SPEC_CAPTURE="$SHELLSPEC_TMPDIR/bus_payload"
    ensure_dispatcher_running
    When run topic --path "$ZSHMQ_CTX_ROOT" send --topic bus 'system overload'
    ...
    wait "$TOPIC_SEND_SPEC_READER_PID" 2>/dev/null || :
    TOPIC_SEND_SPEC_READER_PID=
    The contents of file "$TOPIC_SEND_SPEC_CAPTURE" should equal 'PUB|bus|system overload'
  End

  It 'logs the dispatched message when trace logging is enabled'
    TOPIC_SEND_SPEC_CAPTURE="$SHELLSPEC_TMPDIR/bus_payload"
    ensure_dispatcher_running
    When run topic --path "$ZSHMQ_CTX_ROOT" send --topic bus --trace 'system overload'
    The status should be success
    The stdout should equal ''
    The stderr should include '[TRACE] topic send: topic=bus message=system overload'
    The stderr should not include '[INFO] topic send:'
    wait "$TOPIC_SEND_SPEC_READER_PID" 2>/dev/null || :
    TOPIC_SEND_SPEC_READER_PID=
  End
  ...
  It 'fails when the dispatcher is not running'
    ctx --path "$ZSHMQ_CTX_ROOT" new >/dev/null 2>&1
    topic --path "$ZSHMQ_CTX_ROOT" new -T bus >/dev/null 2>&1
    When run topic --path "$ZSHMQ_CTX_ROOT" send --topic bus 'no loop'
  ...
  It 'allows message payloads containing pipes'
    TOPIC_SEND_SPEC_CAPTURE="$SHELLSPEC_TMPDIR/bus_payload"
    ensure_dispatcher_running
    When run topic --path "$ZSHMQ_CTX_ROOT" send --topic bus 'value | with | pipes'
    The status should be success
    The stdout should equal ''
    The stderr should not include '[INFO] topic send:'
    The stderr should not include '[TRACE]'
    wait "$TOPIC_SEND_SPEC_READER_PID" 2>/dev/null || :
    TOPIC_SEND_SPEC_READER_PID=
    The contents of file "$TOPIC_SEND_SPEC_CAPTURE" should equal 'PUB|bus|value | with | pipes'
  End
End
